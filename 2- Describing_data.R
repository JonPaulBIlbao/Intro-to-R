...
# Table as an object
# ==========
p1.table <- table(as.factor(store.df$p1price))
p1.table
p1.table[3]
str(p1.table)

store.df$p1price <- as.factor(store.df$p1price)
store.df$p2price <- as.factor(store.df$p2price)
store.df$country <- as.factor(store.df$country)
str(store.df)
summary(store.df)
...
quantile(store.df$p1sales, probs=1:20/20)  # shortcut
...
hist(store.df$p1sales, 
     ...
     xlab="Product 1 Sales (Units)",
     ylab="Count",
     breaks=50,             # more columns 
     col="green")       # color the bars
...
# ==========
boxplot(store.df$p2sales ~ store.df$country, horizontal=TRUE,col="red",
...
install.packages("rworldmap", "RColorBrewer")
